...
struct tcp_hdr
{
    ...
    unsigned char tcp_flags : 4;  // TCP flags (and 2 bits from reserved space)
#define TCP_FIN 0x01
#define TCP_SYN 0x02
#define TCP_RST 0x04
...
/* This function accepts a socket FD and ptr to the null terminated
...
/* This function accepts a socket FD and a ptr to a destination
    buffer. It will receive from the socket until EOL byte
    sequence is seen. The EOL bytes are read from the socket, but
    the destination buffer is terminated before these bytes.
    Returns the size of read line (without EOL bytes).
*/
